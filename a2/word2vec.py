...
def naiveSoftmaxLossAndGradient(
    ...
    y_preds = softmax(np.dot(outsideVectors, centerWordVec)) # (W, )
    loss = -np.log(y_preds[outsideWordIdx]) # float
    gradCenterVec = np.dot(y_preds, outsideVectors) - outsideVectors[outsideWordIdx] # (d, )
    y_preds[outsideWordIdx] -= 1 # modify y_preds to be y_preds - y_true
    gradOutsideVecs = np.outer(y_preds, centerWordVec) #(W, d)
    ### END YOUR CODE
...
def negSamplingLossAndGradient(
    ...
    y_preds = sigmoid(np.dot(outsideVectors, centerWordVec)) # (W, )
    y_preds_neg = sigmoid( - np.dot(outsideVectors, centerWordVec)) # (W, )
    loss = - np.sum(np.log(1 - y_preds[negSampleWordIndices])) - np.log(y_preds[outsideWordIdx]) # float
    gradCenterVec = (y_preds[outsideWordIdx] - 1) * outsideVectors[outsideWordIdx] + \
                    np.dot(y_preds[negSampleWordIndices], outsideVectors[negSampleWordIndices]) # (d, )
    gradOutsideVecs = np.zeros(outsideVectors.shape) #initializing
    gradOutsideVecs[outsideWordIdx,:] = - centerWordVec * (1 - y_preds[outsideWordIdx]) 
    for ele in negSampleWordIndices:
        gradOutsideVecs[ele,:] += (1 - y_preds_neg[ele]) * centerWordVec # count the grads
    ### END YOUR CODE
...
def skipgram(currentCenterWord, windowSize, outsideWords, word2Ind,
    ...
    for outsideWordIdx in outsideWordIdxs[0 : 2 * windowSize]:
        l, gcv, gov = word2vecLossAndGradient(currentCenterWordVec,
                                              outsideWordIdx, 
                                              outsideVectors, 
                                              dataset)
        loss += l
