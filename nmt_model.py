...
class NMT(nn.Module):
    ...
    def __init__(self, embed_size, hidden_size, vocab, dropout_rate=0.2):
        ...
        self.target_vocab_projection = nn.Linear(in_features=hidden_size, out_features=len(vocab.tgt), bias=False) # V_t = len(vocab.tgt)
    ...
    def encode(self, source_padded: torch.Tensor, source_lengths: List[int]) -> Tuple[torch.Tensor, Tuple[torch.Tensor, torch.Tensor]]:
        ...
        enc_hiddens, _ = pad_packed_sequence(sequence=enc_hiddens, batch_first=True) # T * B * N -> B * T * N
        # 3)
    ...
    def step(self, Ybar_t: torch.Tensor,
        ...
        # 3) (b, src_len, h) * [(b, h) -> (b, h, 1)] -> (b, src_len, 1) -> (b, src_len)
        ...
        # 2) [(b, src_len) -> (b, 1, src_len)] * (b, src_len, 2h) -> (b, 1, 2h) -> (b, 2n)
