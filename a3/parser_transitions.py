...
class PartialParse(object):
    ...
    def parse_step(self, transition):
        ...
        ###         3. Right Arc
        # transition legal
        assert transition in ['S', 'LA', 'RA'], print('Illegal transition: ', transition)
        # 1. SHIFT
        if transition == 'S':
...
def minibatch_parse(sentences, model, batch_size):
    ...
    # Initialize unfinished parses: shallow copy
    unfinished_parses = partial_parses[:]
    def is_finished(parse):
        return bool((not parse.buffer) and len(parse.stack) == 1)
    while len(unfinished_parses) > 0:
        # Use the model to predict the next transition for each partial parse in the minibatch
        batch_parses = unfinished_parses[:batch_size]
        transitions = model.predict(batch_parses)
        # Perform a parse step on each partial parse in the minibatch with its predicted transition
        for trans, parse in zip(transitions, batch_parses):
            parse.parse_step(trans)
        # Remove the completed (empty buer and stack of size 1) parses from unfinished parses
        unfinished_parses = [parse for parse in unfinished_parses if not is_finished(parse)]
    # load dependencies
