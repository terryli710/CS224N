...
class SynthesizerAttention(nn.Module):
    ...
    def forward(self, x, layer_past=None):
        ...
        # calculate query, key, values for all heads in batch and move head forward to be the batch dim
        relu = nn.ReLU()
        att = relu(self.w1(x).view(B, T, self.n_head, C // self.n_head).transpose(1, 2)) # (B * l * d) @ (d * d) -> (B * l * d) -> (B * l * (nh * hs)) -> (B, nh, T, hs)
        att = att  @ self.w2 # (B, nh, l, hs) @ (hs * l) -> (B, nh, l, l)
        att = F.softmax(att + self.b2, dim=-1) # (B, nh, l, l) + l -> soft((B, nh, l, l)) -> (B, nh, l, l)
        att = self.attn_drop(att)
