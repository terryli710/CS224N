...
class CharCorruptionDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # 1. Randomly truncate the document to a length no less than 4 characters,
        # and no more than int(self.block_size*7/8) characters.
        trunk_len = random.randint(4, int(self.block_size*7/8))
        truncated_doc = document[:trunk_len]
        ...
        # The length of [masked_content] should be random, and 1/4 the length of the
        #     truncated document on average.
        mask_len = int(random.gauss(1/4 * len(truncated_doc), 2))
        if mask_len <= 0: mask_len = 1
        elif mask_len > len(truncated_doc): mask_len = len(truncated_doc) - 2
        # randomly pick prefix_len to be >= 1
        prefix_len = int(random.uniform(1, len(truncated_doc) - mask_len - 1))
        prefix = truncated_doc[:prefix_len]
        masked_content = truncated_doc[prefix_len: prefix_len + mask_len]
        suffix = truncated_doc[prefix_len + mask_len:]
        ...
        # string to be masked_string[1:]. In other words, for each character, the goal is
        # to predict the next character in the masked string.
        # pads should be one char shorter than previous implementation
        pads = self.PAD_CHAR*(self.block_size - len(masked_string))
        x = masked_string[:-1] + pads
        y = masked_string[1:] + pads
